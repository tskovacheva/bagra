...
async function planBlock(c, recipes, substances) {
  const byId = new Map(recipes.map(r => [r.id, r]));
  const subById = new Map(substances.map(s => [s.id, s]));
  // Expanded, not merely scaled: a step the plan is not correct without is part
  // of the work, and has to be shown where it is done and with what it needs.
  const steps = expandChain(c, byId, ctx);

  // Numbered before the map, not inside it. A counter incremented in an async
  // body runs after the first await, so the numbers would be handed out in
  // whatever order the promises happened to settle.
  let counter = 0;
  const numbered = steps.map(st => ({ st, n: st.required ? null : ++counter }));

  const blocks = await Promise.all(numbered.map(async ({ st, n }) => {
    if (!st.recipe) return '';
    ...
    // A required step is not numbered. The numbers belong to the plan the owner
    // built; giving the chalk bath a "4" would renumber her chain every time a
    // mordant changed. It carries a turned arrow instead, and says plainly
    // that it is not optional.
    return `
      <div class="planstep${st.required ? ' required' : ''}">
        <div class="chainhead">
          <span class="stepnum${st.required ? ' followmark' : ''}"
            ${st.required ? `title="${esc(t('chains.requiredAfter', { n: st.after }))}"` : ''}
            >${st.required ? '&#8627;' : n}</span>
          <b>${esc(text(st.recipe.name))}</b>
          <span class="spacer"></span>
          <span class="hint">${esc(conditions)}</span>
        </div>
        ${st.required ? `<p class="requirednote">${t('chains.requiredStep')}</p>` : ''}
      ...
      <div class="cols">
        <div class="col">
          ...
          ${panel(`
            ...
            ${!isNew ? `${actionBtn('delete', t('chains.delete'), 'data-delete-chain', 'destructive')}` : ''}
          `)}
        </div>
        ...
        <div class="col">
          ...
          ${!isNew ? panel(`
            <button class="btn quiet" disabled>${t('chains.makeTrial')}</button>
